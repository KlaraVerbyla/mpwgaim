...
##' }
##'
summary.mvmpwgaim <- function(object, intervalObj, by = "id", LOGP = TRUE, ...)
{
...
#    require(MASS)  ## delete - ABZ
    ...
    if(object$QTL$main)
        ...
        qtl.type <- rep("interaction", n.qtl)
    ...
    effects <- Pvalue <- LogProb <- i.Pvalue <- i.LogProb <-
        Trait.list <- qtl.int <- founder.list <- list()
    #
    #  Main effect QTL
    #
    if(length(qtl.m)>0) {
        ...
        for (ii in 1:length(qtl.m)) {
            ...
            names(qlist) <- qtl.ii
            pred <- predict(object, classify = paste0('grp(',qtl.ii,')'), only = paste0('grp(',qtl.ii,')'),
            ...
            Pvalue[[ii]] <- rep(pchisq(d2.ii, df = nfounders-1, lower.tail=FALSE), n.trait)
            LogProb[[ii]] <- round(-log(Pvalue[[ii]], base=10), 2)
            i.d2.ii <- rep((effects[[ii]]^2) / diag(vcov), n.trait)
            i.Pvalue[[ii]] <- pchisq(i.d2.ii, df = 1, lower.tail=FALSE)/2
            i.LogProb[[ii]] <- round(-log(i.Pvalue[[ii]], base=10), 2)
            Trait.list[[ii]] <- rep(object$QTL$trait.levels, each=nfounders)
            founder.list[[ii]] <- rep(pred$pvals[, qtl.ii], n.trait)
            ...
            qtl.int[[ii]] <- rep(paste(wchr, wint, sep='.'), nfounders*n.trait)
        }
    }
    n.main <- length(qtl.m)
    #
    ...
    if(length(which3) > 0) {
        ...
        for (ii in 1:length(which3)) {
            ...
            if(!object$QTL$main) znt.me.ii <- NULL
            pred.ii <- predict(object, classify = which2[ii], only=c(which2[ii], znt.me.ii),
                               levels=qlist, maxiter=1, vcov=TRUE, data=phenoData)$predictions
            ...
            for (kk in 1:n.trait) {
                ...
                d2.ii <- sum(effects[[jj]][kk.left:kk.right] %*% ginv(vcov[kk.left:kk.right,
                                                                           kk.left:kk.right]) *
                             effects[[jj]][kk.left:kk.right])
                Pvalue[[jj]] <- c(Pvalue[[jj]], pchisq(d2.ii, df = nfounders-1, lower.tail=FALSE))
            }
            Pvalue[[jj]] <- Pvalue[[jj]][-1]
            LogProb[[jj]] <- round(-log(Pvalue[[jj]], base=10), 2)
            i.d2.ii <- (effects[[jj]]^2) / diag(vcov)
            i.Pvalue[[jj]] <- pchisq(i.d2.ii, df = 1, lower.tail=FALSE)/2
            i.LogProb[[jj]] <- round(-log(i.Pvalue[[jj]], base=10), 2)
            Trait.list[[jj]] <- object$QTL$trait.levels[as.numeric(pred.ii$pvals[, trait])]
            founder.list[[jj]] <- pred.ii$pvals[, qtl.i[ii]]
            tmp.split <- strsplit(qtl.i[ii], split="\\.")
            wchr <- unlist(lapply(tmp.split, function(el) el[2]))
            wint <- as.numeric(unlist(lapply(tmp.split, function(el) el[3])))
            qtl.int[[jj]] <- rep(paste(wchr, wint, sep='.'), nfounders*n.trait)
        }
    }
    ##
    ...
    ##
    qtlmat <- data.frame(int=unlist(qtl.int), wchr = wchr, wint = wint, unlist(Trait.list), Founder= unlist(founder.list),
                         Size = unlist(effects), Founder.Prob = unlist(i.Pvalue), Founder.LOGP = unlist(i.LogProb))
    ...
    for (ii in 1:length(object$QTL$qtl.list)) {
      ...
      for (kk in 1:n.trait) {
        kk.left <- (kk-1)*nfounders+1
        kk.right <- kk*nfounders
        qtl.var[kk,ii] <- sum((effects[[ii]][kk.left:kk.right] %*% (diag(av.prob) - prob.mat)) *
                              effects[[ii]][kk.left:kk.right])
       }
    }
    coeff.other <- sum(apply(gD, 2, mean)^2)
    poly <- sigma2 * object$gammas[grep(paste(by, "!", sep = ""), names(object$gammas))]
    ints <- sigma2 * object$gammas[grep(paste("ints", "!", sep = ""), names(object$gammas))]/10000
    if(n.trait > 2) {
      n.fa <- length(poly)/n.trait - 1
      p.psi <- poly[1:n.trait]
      p.lambda <- matrix(poly[(n.trait+1):length(poly)], ncol=n.fa, byrow=FALSE)
      p.var <- diag(p.lambda %*% t(p.lambda) + diag(p.psi))
      n.fa <- length(ints)/n.trait - 1
      i.psi <- ints[1:n.trait]
      i.lambda <- matrix(ints[(n.trait+1):length(ints)], ncol=n.fa, byrow=FALSE)
    ...
    if(n.trait == 2) {
      ...
      p.var[1,1] <- poly[2]
      p.var[2,2] <- poly[3]
      p.var[1,2] <- p.var[2,1] <- poly[1] * sqrt(poly[2]*poly[3])
      p.var <- diag(p.var)
      i.var[1,1] <- ints[2]
      i.var[2,2] <- ints[3]
      i.var[1,2] <- i.var[2,1] <- ints[1] * sqrt(ints[2]*ints[3])
      i.var <- diag(i.var)
    }
    ...
    qtlmat[, "Founder.Prob"] <- round(qtlmat[, "Founder.Prob"],4)
    qtlmat[, "% var"] <- round(qtlmat[, "% var"],1)
    qtlmat[, "LOGP"] <- round(qtlmat[, "LOGP"],1)
    qtlmat <- qtlmat[order(qtlmat[,"wchr"], qtlmat[,"wint"], qtlmat[,trait], qtlmat[, "Founder"]),]
    ...
    wchr <- unlist(lapply(tmp.split, function(el) el[2]))
    wint <- as.numeric(unlist(lapply(tmp.split, function(el) el[3])))
    intmat <- getmvQTL(object, intervalObj)
    ...
    if (object$QTL$type == "interval") {
      names(qtl.table)[1:5] <- c("Chromosome", "Left Marker", "dist (cM)",
                                 "Right Marker", "dist (cM)")
      traitpos <- 6
      fndrpos <- 7
    }
    else {
      names(qtl.table)[1:3] <- c("Chromosome", "Marker", "dist (cM)")
      traitpos <- 4
      fndrpos <- 5
    }
    qtl.table <- qtl.table[order(qtl.table[, "Order"]),]
    qtl.table[, "Order"] <- NULL
    qtl.table[, fndrpos] <- intervalObj$founders[qtl.table[, fndrpos]]
    oseqs <- c(1:(n.trait*nfounders*n.qtl))[!(1:(n.trait*nfounders*n.qtl) %in%
                                              seq(from=1, to=(n.trait*nfounders*n.qtl),
                                                  by=nfounders))]
    oseqs2 <- c(1:(n.trait*nfounders*n.qtl))[!(1:(n.trait*nfounders*n.qtl) %in%
                                               seq(from=1, to=(n.trait*nfounders*n.qtl),
                                                   by=(nfounders*n.trait)))]
